...
| `crates/graphify-extract/src/stubs.rs` | ExternalStubs prefix matcher (settings + project lists chained, longest-prefix wins) |
...
- **Self-dogfood UX rule (FEAT-043 close)**: when running `graphify suggest stubs` on this repo and the candidate list contains internal symbols misclassified as external (BUG-022 surfaced ~30: `PolicyError`, `walker::*`, `src.Community`, `manifest::sha256_of_bytes`, `pct`, `matches`, etc.) — **do NOT add them to `external_stubs` to silence the symptom**. Adding them masks the underlying graphify-resolver bug. Add only legitimate externals (real workspace siblings, real third-party deps, Rust macros). The 5 dogfood adds applied at FEAT-043 close (`include_str`, `graphify_core/extract/report`, `anstyle`) are the template; everything else needs root-cause investigation in the resolver
