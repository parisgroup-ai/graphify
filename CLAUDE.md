...
- Type alias collapse (`pub type Cycle = Vec<String>;`) is NOT covered by FEAT-045/046/047 — those handle `pub use` declarations only. `src.Cycle` and similar `pub type X = Y;` aliases stay in `graphify suggest stubs` candidates until FEAT-049 lands. Recommended shape (per FEAT-049 task body): treat `pub type X = Y;` as a `ReExportEntry` with `raw_target = full_path_of_Y` and reuse the same pipeline (option 1, data-quality fix); the alternative (filter `pub type` from `score_stubs` only) is a UX band-aid that doesn't fix `analysis.json` edge counts
