...
- **`is_local` and per-node `language` live in `graph.json`, NOT `analysis.json`** — `analysis.json` carries `MetricsRecord` (id, scores, in_degree, out_degree, in_cycle, community_id, hotspot_type) plus an `edges[]` array. Tooling that needs node-level provenance must read `graph.json`.
...
- Defines-target seeding (BUG-018): before edge resolution, `run_extract_with_workspace` (graphify-cli) and `run_extract` (graphify-mcp) iterate `all_raw_edges` and `resolver.register_module(raw_target)` for every `EdgeKind::Defines` — seeds `known_modules` with symbol-level ids so local Calls don't get capped to `0.5/Ambiguous`
- Resolver case 8.5 (BUG-019): bare identifiers (`no ::, ., \, /, leading dot`) + non-empty `from_module` → synthesize `{from_module}.{raw}` and look it up in `known_modules` *before* the use-alias fallback. Local-first matches Rust shadowing. Language-agnostic; covers Rust/Go/PHP. **Python/TS don't need this — they filter same-file helpers at extraction via `collect_imported_bindings`. This asymmetry is load-bearing — do not "harmonize" the extractors in either direction.**
- Resolver case 8.6 (BUG-022): Rust-shaped scoped paths (`contains :: AND no ., \, /, leading dot`) + non-empty `from_module` synthesize `{from_module}.{raw with :: → .}`. Same shape as 8.5 for scoped input. Covers same-file `Type::method` and sibling-mod from crate root. Ordered before case 9 (use-alias fallback) for shadowing semantics
- BUG-023 / BUG-024 / BUG-025 (Rust extractor scope discipline): `collect_use_paths` recurses through nested grouped use-lists; `extract_calls_recursive` takes a `local_bindings: &HashSet<String>` to skip closure/let/nested-fn callees; `walk_for_uses` extracts function-body `use_declaration`s. Skip discipline: descent into nested `function_item`/`impl_item` collects the name then returns without entering the body — bindings/aliases don't leak across fns. Per-file `use_aliases` is per-file-not-per-scope (last-write-wins; v2 refactor deferred). Details: `docs/TaskNotes/Tasks/BUG-{023,024,025}-*.md`
- BUG-027 / FEAT-049 (Rust local-symbol Defines emission): `extract_file` arms for `static_item`/`const_item`/`type_item` and per-variant Defines for `enum_item` emit `Defines` edges at `{module}.{Sym}` (and `{module}.{Enum}.{Variant}`) so BUG-018 seeding registers them in `known_modules`. Visibility never gates Defines emission. **NodeKind reuse**: `Class` for type aliases — adding `TypeAlias` cascades through every report writer
- BUG-026 lesson: when `graphify suggest stubs` flags an apparent extractor regression, `jq` the graph FIRST. Surfaced as a `std::env::*`-loses-prefix hypothesis; the actual graph already classified those correctly via the `std` stub — the bug was 2 unrelated `env!("CARGO_PKG_VERSION")` macro callsites resolved by adding `env` to `[settings].external_stubs`
...
- `analysis.json` carries top-level `generated_at` (ISO 8601 UTC, format `YYYY-MM-DDTHH:MM:SSZ`) since 0.13.7 (BUG-028). `graphify session brief::baseline_age_days` reads it from the freshest `analysis.json` under `report/baseline/` (depth ≤ 1, picks the youngest age across multi-project layouts). Falls back to file mtime when the field is absent (legacy 0.13.6 snapshots). **Do not use directory mtimes for content-freshness checks** — POSIX dir mtimes only update on entry add/remove, so in-place file overwrites (the standard `cp` baseline-promotion gesture) leave them stale. Stamp helper lives in `graphify-report::time_utils::now_iso8601` (chrono-free, shared with `consolidation.rs`)
...
- TS barrel re-export collapse (FEAT-021/025/026): `run_extract` aggregates per-file `ReExportEntry`s into a project-wide `ReExportGraph` and walks every TS symbol to canonical. Outcomes — `Canonical`: drop barrel symbol nodes, append dropped + intermediate ids to canonical's `alternative_paths`; `Cycle`: stderr log, leave participants; `Unresolved`: `Info:` stderr log, leave node as-is (no confidence downgrade). TS extractor captures named + default import specifiers via `NamedImportEntry` and fans out per specifier; `import * as ns` stays a single barrel edge; type-only imports get `is_type_only: true`. Same-project tsconfig aliases covered post-FEAT-026
- Cross-project TS aliases (FEAT-028): `@repo/*` spanning sibling `[[project]]`s fan out via `WorkspaceReExportGraph` (`crates/graphify-extract/src/workspace_reexport.rs`). Built only when ≥2 projects AND ≥1 TS project. Pipeline split: `build_project_reexport_context` (phase 1, collect) + `run_extract_with_workspace` (phase 2, fan-out). Public node ids stay per-project; cross-project edges reference target's `module_id` verbatim (first-wins via `modules_to_project`). `match_alias_target` supports inner-glob tsconfig forms. Opt-out: `[settings] workspace_reexport_graph = false` (ADR `docs/adr/0001-workspace-reexport-graph-gate.md`)
- Rust intra-crate `pub use` collapse (FEAT-045/046/047): mirrors TS FEAT-021/025/026 for Rust. **Asymmetry vs TS** — Rust `pub use foo::Bar;` does NOT create a barrel symbol node (extractor emits an `Imports` edge, not a node), so the TS "drop barrel + append `alternative_paths`" step is a NO-OP for Rust; the load-bearing piece is consumer-side edge-target rewrite via `barrel_to_canonical` (exact-match AND prefix-match shapes) plus `ModuleResolver::rewrite_use_alias_targets` BEFORE case 9 fallback. Wildcards / single-segment `pub use` are v1 no-ops. Design: `docs/superpowers/specs/2026-04-26-feat-044-rust-reexport-collapse-design.md`
- Cross-crate Rust `pub use` workspace fan-out (FEAT-048) deferred via gate. Threshold ≥5 cross-crate misclassifications in `graphify suggest stubs`; current 1 hit (`graphify-report`'s `pub use graphify_core::community::Community;`). Plumbing would mirror TS FEAT-028. ADR: `docs/adr/0002-cargo-workspace-reexport-graph-gate.md`
...
- `graphify suggest stubs` (FEAT-043): post-analysis subcommand consuming each project's `graph.json` (NOT `analysis.json` — needs `is_local` per node and `weight` per link). Pure scorer + 3 renderers in `graphify_report::suggest`; CLI orchestration + `--apply` (`toml_edit::DocumentMut` + atomic rename, idempotent) in `graphify_cli::main::cmd_suggest_stubs`. Auto-classify: prefix surviving `--min-edges` per-project in ≥2 projects → `[settings]`; in 1 project → that project's block. Language-aware prefix extraction (Rust first `::`-segment, Python/PHP first `.`-segment, TS scoped `@scope/name` else first `/`-segment, Go path-style 3 segments if seg-0 contains `.` else first `.`-segment). Shadowing safety: matches against `local_prefix` + top-segments of every `is_local=true` node id never suggested. Threshold applied per-project BEFORE cross-project aggregation. `--apply` conflicts with `--format`
- `ExternalStubs` lives in `graphify-core` (CHORE-011): pure prefix matcher, no extractor coupling. Public path `graphify_core::ExternalStubs`. Old `graphify_extract::ExternalStubs` re-exports are GONE — no shim. `graphify-report` no longer depends on `graphify-extract`. **Heuristic gotcha**: `graphify-summary.json` still surfaces a `graphify-report → graphify-extract` edge count (~80) — name-based overlap, not Cargo-dep direction
- `ExternalStubs::matching_prefix(&self, target) -> Option<&str>` (BUG-021): companion to `matches()` returning the actual stub that matched. Leverages longest-first sort in `new()` so first hit = longest match (`tokio::runtime` wins over `tokio` for `tokio::runtime::Builder`). Used by `score_stubs` to record the precise stub in `already_covered_prefixes`
- **Self-dogfood UX rule**: when `graphify suggest stubs` flags internal symbols misclassified as external — **do NOT add them to `external_stubs` to silence the symptom**. Adding masks underlying graphify bugs. Add only legitimate externals (workspace siblings, third-party deps, language macros). Fix the root extractor/resolver bug instead
- **Workspace siblings as `external_stubs` (FEAT-043 dogfood)**: `[settings].external_stubs` includes `graphify_core`, `graphify_extract`, `graphify_report` because each crate sees its siblings as external. Pattern generalizes to any Cargo workspace analyzed crate-by-crate — each `[[project]]` should declare cross-crate siblings
