...
### Added
- feat(extract): Rust consumer-side `use_aliases` canonicalization (FEAT-047). After FEAT-046 builds `barrel_to_canonical`, `run_extract_with_workspace` now calls `ModuleResolver::rewrite_use_alias_targets` to repoint per-file `use_aliases_by_module` values whose resolved form matches a barrel id. The substitution preserves Rust syntax (`crate::Bar` → `crate::foo::Bar`) so case 9's scoped-form recursion still flows through case 6 (`crate::` strip) — substituting a bare dot-notation id directly would yield hybrid `src.foo.Bar::new` targets that no resolver branch handles. With the alias map canonicalized, the resolver returns the canonical declaration directly at confidence 1.0 instead of routing through the barrel and relying on FEAT-046's post-resolve edge rewrite to fix the target. Functionally a no-op end-state given FEAT-046 is in place (the same canonical id lands on the edge both ways), but defensive against future resolver paths that consume `use_aliases` without the post-resolve hook. Per-file scope limitation documented inline (BUG-025 trade-off — `use_aliases_by_module` is keyed by `from_module`, so a file with both `use crate::Bar;` and `use other::Bar;` last-write-wins). Out of scope: cross-crate `pub use` fan-out (FEAT-048, gated), type-alias collapse (FEAT-049 candidate). Dogfood baseline holds: `graphify suggest stubs` candidate count steady at 7, cycles still 0, no hotspot >0.85. The remaining `src.Community` and `src.Cycle` candidates are cross-crate / type-alias-shaped and only drop when FEAT-048 / FEAT-049 ship.
...
### Fixed
- fix(report): `suggest stubs` no longer misreports the "already covered" prefixes when a stub guards only a sub-namespace (BUG-021). Pre-fix, `score_stubs` recorded `extract_prefix(target, lang)` (the top-level segment) whenever a stub matched — so a `tokio::runtime` stub falsely surfaced as covering all of `tokio`. New method `ExternalStubs::matching_prefix(&str) -> Option<&str>` (in `crates/graphify-core/src/stubs.rs`) returns the actual stub that matched, leveraging the existing longest-first sort in `new()` so the first hit is also the longest. `score_stubs` now records the matched stub itself in `already_covered_prefixes`. Behavior unchanged for the dogfood case (graphify-self has no sub-namespace stubs); fix is defensive against a class of configs that could have produced misleading reports. 4 new tests cover longest-match priority, no-leak asymmetry, no-match return value, and the integration regression in `score_stubs`.
